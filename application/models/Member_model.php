...
class Member_model extends CRUD_Model
{
	...
	{
		$this->db->select('*');
		$this->db->from('mem_subscriptions');
		$this->db->where("mem_id", $mem_id);
    ...
    {
        $this->db->select('m.mem_id, m.mem_type, m.mem_fname, m.mem_lname, m.mem_display_name, m.mem_email, m.mem_phone, m.mem_image, m.mem_address, m.mem_verified, m.mem_status, m.mem_professionl_profile, m.mem_specialization, m.mem_bio, m.mem_phone_verified');
        $this->db->from('members m');
        $this->db->join('professional_profile pp', 'm.mem_id = pp.mem_id');
        $this->db->where('pp.phone_verified', 1);
    ...
    function search_pro_members($data, $radius = 10){
        ...
        if(!empty($data['search_name'])){
            ...
            $this->db->like('m.mem_fname', $data['search_name'], 'after'); // Use 'after' for prefix search
        ...
        if(!empty($data['service_id'])){
            ...
            $this->db->where('pp.service_id', $data['service_id']);
        ...
        if(!empty($data['sub_service_id'])){
            $this->db->group_start();
            $this->db->join('tbl_selected_sub_services ss', 'pp.id = ss.mem_profile_id');
            
            $this->db->where('ss.sub_service_id', $data['sub_service_id']);
        ...
        if (!empty($data['rating'])) {       
                $this->db->join('reviews r', 'm.mem_id = r.pro_mem_id');
                $this->db->select('AVG(r.rating) AS avg_rating');
                $this->db->group_by('r.pro_mem_id');
                $this->db->having("AVG(r.rating) >= " . intval($data['rating']));        
        }
        ...
        if(!empty($data['completed'])){
            ...
            switch($data['completed']){
                case "1-10":
                    $this->db->where("pp.completed_projects >=", 1 );
                    $this->db->where("pp.completed_projects <=", 10 );
                break;

                case "10-30":
                    $this->db->where("pp.completed_projects >=", 10 );
                    $this->db->where("pp.completed_projects <=", 30 );
                break;

                case "30-50":
                    $this->db->where("pp.completed_projects >=", 30 );
                    $this->db->where("pp.completed_projects <=", 50 );
                break;
        ...
        if(!empty($data['specialization'])){
            ...
            $this->db->where("pp.specialization >=", $data['specialization']);
