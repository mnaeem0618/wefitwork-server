...
{
    $file = fopen("logs/" . $file_name . date('Y-m-d H:i:s') . '.txt', 'w');
    fwrite($file, $post);
}
...
function get_maintenance_cover_service_title($id)
{
    global $CI;
    $CI = get_instance();
    $row = $CI->master->getRow('maintenance_covers', array('id' => $id));
    return ($row->service_title);
}

function get_mem_maintenance_cover_address($id)
{
    global $CI;
    $CI = get_instance();
    $row = $CI->master->getRow('purchased_maint_covers', array('id' => $id));
    return ($row->address);
}

function get_mc_sub_service_title($id)
{
    global $CI;
    $CI = get_instance();
    $row = $CI->master->getRow('mc_sub_services', array('id' => $id));
    return ($row->title);
}
...
function saveBannerImgs($path, $files, $fieldname, $section, $pics, $thumb_size = '')
{
    // pr($files);
    $CI = get_instance();
    $cpt = countlength((array)$files['name']);
    for ($i = 0; $i < $cpt; $i++) {
        // pr($files['name'][$i]);
        if ($files['name'][$i] != '') {
...
function new_mc_requests()
{
    global $CI;
    $CI->db->where('status', 'pending');
    $row = $CI->db->get('mcover_requests');
    return intval($row->num_rows());
}
...
{
    //pr($vals);
    if (countlength((array)$vals['title']) > 0) {
        for ($i = 0; $i < countlength($vals['title']); $i++) {
            // $arr['site_lang'] = $site_lang;
...
function saveIncluded($vals, $maintenance_cover_id)
{
    if (countlength((array)$vals['title']) > 0) {
        for ($i = 0; $i < countlength($vals['title']); $i++) {
            ...
            $CI = get_instance();
            $CI->db->set($arr);
            $CI->db->insert('included');
        }
    }
}

function saveExcluded($vals, $maintenance_cover_id)
{
    if (countlength((array)$vals['title']) > 0) {
        for ($i = 0; $i < countlength($vals['title']); $i++) {
            $arr['maintenance_cover_id'] = $maintenance_cover_id;
            ...
            $CI = get_instance();
            $CI->db->set($arr);
            $CI->db->insert('excluded');
        }
    }
}

function saveMcoverFaqs($vals, $maintenance_cover_id)
{
    //pr($vals);
    if (countlength((array)$vals['question']) > 0) {
        for ($i = 0; $i < countlength($vals['question']); $i++) {
            $arr['maintenance_cover_id'] = $maintenance_cover_id;
            $arr['question'] = ($vals['question'][$i] != '') ? $vals['question'][$i] : '';
            $arr['answer'] = ($vals['answer'][$i] != '') ? $vals['answer'][$i] : '';
            $arr['sort_order'] = ($vals['sort_order'][$i] != '') ? $vals['sort_order'][$i] : '';
            $CI = get_instance();
            $CI->db->set($arr);
            $CI->db->insert('mcover_faqs');
        }
    }
}
...
function getIncluded($maintenance_cover_id)
{
    ...
    // $CI->db->order_by('order_no', 'ASC');
    $query = $CI->db->get('included');
    return $query->result();
}

function getExcluded($maintenance_cover_id)
{
    $CI = get_instance();
    $CI->db->where('maintenance_cover_id', $maintenance_cover_id);
    // $CI->db->order_by('order_no', 'ASC');
    $query = $CI->db->get('excluded');
    return $query->result();
}

function getMcoverFaqs($maintenance_cover_id)
{
    $CI = get_instance();
    $CI->db->where('maintenance_cover_id', $maintenance_cover_id);
