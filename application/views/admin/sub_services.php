<?php if ($this->uri->segment(3) == 'manage'): ?>
    <?= showMsg(); ?>
    <?= getBredcrum(ADMIN, array('#' => 'Add/Update Sub Service')); ?>
    <div class="row margin-bottom-10">
        <div class="col-md-6">
            <h2 class="no-margin"><i class="entypo-list"></i> Add/Update <strong>Sub Service</strong></h2>
        </div>
        <div class="col-md-6 text-right">
            <a href="<?= site_url(ADMIN . '/sub_services'); ?>" class="btn btn-lg btn-default"><i class="fa fa-arrow-left"></i> Cancel</a>
        </div>
    </div>
    <div>
        <hr>
        <div class="row col-md-12">
            <form action="" name="frmTestimonial" role="form" class="form-horizontal" method="post" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <div class="col-md-5">
                        <label class="control-label" for="ser_id"> Choose Service <span class="symbol required">*</span></label>
                        <select name="ser_id" id="ser_id" class="form-control" autocomplete="" required>
                            <option value="">Please Select Service</option>

                            <?php foreach($services as $ser): ?>

                            <option value="<?= $ser->id ?>" <?= $row->ser_id == $ser->id ? 'selected' : '' ?>><?= $ser->title ?></option>
...
<?php else: ?>
    ...
    <div class="row margin-bottom-10">
        ...
        <div class="col-md-6 text-right">
            ...
            <a href="<?= site_url(ADMIN . '/sub_services/manage'); ?>" class="btn btn-lg btn-primary"><i class="fa fa-plus-circle"></i> Add New</a>
        </div>
    </div>
            ...
            <?php if (countlength($rows) > 0): $count = 0; ?>
                <?php foreach ($rows as $row): ?>
                    <tr class="odd gradeX">
                        ...
                        <td><b><?= $row->title ?></b></td>
                        <td><b><?= get_service_title($row->ser_id) ?></b></td>
